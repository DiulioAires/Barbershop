...
async function seedDatabase() {
  ...
  await prisma.booking.deleteMany();  // Exclui os agendamentos primeiro para evitar erros de chave estrangeira
  await prisma.service.deleteMany();  //  Exclui os serviços antes das barbearias
  await prisma.barbershop.deleteMany(); // Exclui as barbearias por último
  try {
    const images = [
    ...
    for (let i = 0; i < 10; i++) {
      ...
      for (const service of services) {
        await prisma.service.create({
          data: {
            NAME: service.name,  // Corrigido para NAME
